...
class TestContract(BoaFixtureTest):
    ...
    def test_Appcall1(self):
        ...
        with self.assertRaises(Exception):
            Compiler.instance().load('example/blockchain/AppCallTest2.py').default

        with self.assertRaises(Exception):
            Compiler.instance().load('example/blockchain/AppCallTest3.py').default

    def test_Appcall2(self):

        output = Compiler.instance().load('example/blockchain/AppCallTest.py').default
        out = output.write()
    ...
    def test_DynamicAppcall(self):
        ...
        output = Compiler.instance().load('example/blockchain/DynamicAppCallTest.py').default
        out = output.write()
